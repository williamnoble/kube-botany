...
func TestPlantIntegration(t *testing.T) {
	...
	t.Logf("Initial state: Stage=%s, AddWater=%d%%, Growth=%d",
	...
	for day := 1; day <= 30; day++ {
		...
		// Log every 5 days
		if day%5 == 0 || day == 1 {
			t.Logf("Day %d: Stage=%s, AddWater=%d%%, Growth=%d",
	...
	for day := 1; day <= 10; day++ {
		...
		t.Logf("Dry Day %d: Stage=%s, AddWater=%d%%, Growth=%d",
			day, p.GrowthStage, p.WaterLevel, p.Growth)
	}
	...
	for day := 1; day <= 5; day++ {
		...
		t.Logf("Recovery Day %d: Stage=%s, AddWater=%d%%, Growth=%d",
			day, p.GrowthStage, p.WaterLevel, p.Growth)
	}
